...
program driver
  ...
  real*8    :: T0, Tf, dTout, Tout, Tcur, rtol, rout(6), t(3), apar(3)
  integer   :: it, Nt, ier, neqn, ndisc, mlmas, mumas, mljac, mujac, idef
  integer   :: ind(1), indsol(8)
...
!!$  rtol = rtols(1)
  ...
  call FARKSetDiagnostics("diags_ark_medakzo.txt", 21, ier);
  if (ier /= 0) then
  ...
  call FARKSetIin('MAX_NSTEPS',      5000, ier)
...
!-----------------------------------------------------------------
  ...
  if (ipar(2) == 0) then
     call jeval(m_dim, i_par(1), t, y, fy, BJac, ier, rpar, i_par)
  else 
     call jeval_i(m_dim, i_par(1), t, y, fy, BJac, ier, rpar, i_par)
  end if
  ier = 0

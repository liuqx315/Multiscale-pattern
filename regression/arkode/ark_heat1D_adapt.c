...
/* choice of linear solver 
   (0=>pcg, 1=>gmres, 2=>bicgstab, 3=>tfqmr, 4=>fgmres) */
#define LS 1

/* choice of using resize or not */
#define NORESIZE
...
int main() {
  ...
  printf("  initial N = %li\n", udata->N);
#if LS == 0
  printf("  PCG linear solver\n");
#elif LS == 1
  printf("  GMRES linear solver\n");
#elif LS == 2
  printf("  BICGSTAB linear solver\n");
#elif LS == 3
  printf("  TFQMR linear solver\n");
#elif LS == 4
  printf("  FGMRES linear solver\n");
#endif
...
#ifdef RESIZE      /* "Resize" operating mode */


  printf("\n  **Resize operating mode**\n\n");
...
#elif LS == 1 /* GMRES */
  flag = ARKSpgmr(arkode_mem, 0, N);
  if (check_flag(&flag, "ARKSpgmr", 1)) return 1;
#elif LS == 2 /* BICGSTAB */
  flag = ARKSpbcg(arkode_mem, 0, N);
  if (check_flag(&flag, "ARKSpbcg", 1)) return 1;
#elif LS == 3 /* TFQMR */
  flag = ARKSptfqmr(arkode_mem, 0, N);
  if (check_flag(&flag, "ARKSptfqmr", 1)) return 1;
#elif LS == 4 /* FGMRES */
  flag = ARKSpfgmr(arkode_mem, 0, N);
  if (check_flag(&flag, "ARKSpfgmr", 1)) return 1;
#endif
  ...
  while (t < Tf) {
    ...
    if (check_flag(&flag, "ARKPcg", 1)) return 1;
#elif LS == 1 /* GMRES */
    flag = ARKSpgmr(arkode_mem, 0, udata->N);
    if (check_flag(&flag, "ARKSpgmr", 1)) return 1;
#elif LS == 2 /* BICGSTAB */
    flag = ARKSpbcg(arkode_mem, 0, udata->N);
    if (check_flag(&flag, "ARKSpbcg", 1)) return 1;
#elif LS == 3 /* TFQMR */
    flag = ARKSptfqmr(arkode_mem, 0, udata->N);
    if (check_flag(&flag, "ARKSptfqmr", 1)) return 1;
#elif LS == 4 /* FGMRES */
    flag = ARKSpfgmr(arkode_mem, 0, udata->N);
    if (check_flag(&flag, "ARKSpfgmr", 1)) return 1;
#endif
    flag = ARKSpilsSetJacTimesVecFn(arkode_mem, Jac);
    if (check_flag(&flag, "ARKSpilsSetJacTimesVecFn", 1)) return 1;
...
#else   /* destroy/recreate operating mode */


  printf("\n  **Destroy/Recreate operating mode**\n\n");

  /* Main time-stepping loop: calls ARKode to perform the integration, then
     prints results.  Stops when the final time has been reached */
  realtype t = T0;
  realtype olddt, newdt=0.0;
  printf("  iout          dt_old                 dt_new               ||u||_rms       N   NNI  NLI\n");
  printf(" ----------------------------------------------------------------------------------------\n");
  printf(" %4i  %19.15e  %19.15e  %19.15e  %li   %2i  %3i\n", 
	 iout, olddt, newdt, sqrt(N_VDotProd(y,y)/udata->N), udata->N, 0, 0);
  ...
  while (t < Tf) {
    ...
    /* Initialize the integrator memory */
    flag = ARKodeInit(arkode_mem, NULL, f, t, y);
    if (check_flag(&flag, "ARKodeInit", 1)) return 1;
    ...
    if (check_flag(&flag, "ARKodeSetInitStep", 1)) return 1;
    if (iout > 0) {
      flag = ARKodeSetMaxFirstGrowth(arkode_mem, 20.0);
    ...
    iout++;
    printf(" %4i  %19.15e  %19.15e  %19.15e  %li   %2li  %3li\n", 
	   iout, olddt, newdt, sqrt(N_VDotProd(y,y)/udata->N), udata->N, nni, nli);

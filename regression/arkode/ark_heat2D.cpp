...
int main(int argc, char* argv[]) {
  ...
  int imex;                      // flag denoting class of method to use
  ...
  void *arktrue_mem = NULL;      // empty ARKode memory structure for reference solution
  ...
  flag = MPI_Bcast(dbuff, 2, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  if (check_flag(&flag, "MPI_Bcast", 1)) return 1;
  ...
  FILE *DFID;
  if (outproc)
    DFID=fopen("diags_ark_heat2D.txt","w");
  ...
  N_VConst(0.0, ytrue);                              // Set initial conditions 
  ...
  arktrue_mem = ARKodeCreate();                      // Create the solver memory for reference
  ...
  flag = ARKodeSetUserData(arktrue_mem, (void *) udata);  // Pass udata to user functions 
  if (check_flag(&flag, "ARKodeSetUserData", 1)) return 1;
  if (outproc) {
    flag = ARKodeSetDiagnostics(arkode_mem, DFID);        // Specify tolerances 
  ...
  flag = ARKPcg(arktrue_mem, 1, 20);                            // Specify the PCG solver 
  if (check_flag(&flag, "ARKPcg", 1)) return 1;
  flag = ARKSpilsSetPreconditioner(arktrue_mem, PSet, PSol);    // Specify the Preconditoner
  if (check_flag(&flag, "ARKSpilsSetPreconditioner", 1)) return 1;
  ...
  for (iout=0; iout<Nt; iout++) {
    ...
    flag = ARKode(arktrue_mem, tout, ytrue, &t2, ARK_NORMAL);   // call integrator 
    if (!idense)
  ...
  N_VDestroy_Parallel(ytrue);
  N_VDestroy_Parallel(yerr);

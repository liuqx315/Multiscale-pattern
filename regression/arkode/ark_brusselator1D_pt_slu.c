...
int main(int argc, char *argv[])
{
  ...
  /* Create Pthreads vector of length NEQ for initial condition */
  y = N_VNew_Pthreads(NEQ, num_threads);
  if (check_flag((void *) y, "N_VNew_Pthreads", 0)) return 1;
  ytrue = N_VNew_Pthreads(NEQ, num_threads);
  if (check_flag((void *) ytrue, "N_VNew_Pthreads", 0)) return 1;
  yerr = N_VNew_Pthreads(NEQ, num_threads);
  if (check_flag((void *) yerr, "N_VNew_Pthreads", 0)) return 1;
  ...
  /* Create Pthreads vector masks for each solution component */
  umask = N_VNew_Pthreads(NEQ, num_threads);
  if (check_flag((void *) umask, "N_VNew_Pthreads", 0)) return 1;
  vmask = N_VNew_Pthreads(NEQ, num_threads);
  if (check_flag((void *) vmask, "N_VNew_Pthreads", 0)) return 1;
  wmask = N_VNew_Pthreads(NEQ, num_threads);
  if (check_flag((void *) wmask, "N_VNew_Pthreads", 0)) return 1;
  ...
  N_VDestroy_Pthreads(ytrue);
  N_VDestroy_Pthreads(yerr);
  N_VDestroy_Pthreads(umask);
  N_VDestroy_Pthreads(vmask);
  N_VDestroy_Pthreads(wmask);

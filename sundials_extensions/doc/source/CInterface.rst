...
10. Get optional outputs 
   ...
   :ref:`Fcn.ARKBandPrecGetWorkSpace` and
   :ref:`Fcn.ARKBandPrecGetNumRhsEvals`.  
...
.. _Fcn.ARKBandPrecInit:

``ARKBandPrecInit``
"""""""""""""""""""""""""""""""
:Definition: ``int ARKBandPrecInit(void *arkode_mem, long int N, long int mu, long int ml)`` 
...
.. _Fcn.ARKBandPrecGetWorkSpace:

``ARKBandPrecGetWorkSpace``
"""""""""""""""""""""""""""""""
:Definition: ``int ARKBandPrecGetWorkSpace(void *arkode_mem, long int *lenrwLS, long int *leniwLS)`` 
...
.. _Fcn.ARKBandPrecGetNumRhsEvals:

``ARKBandPrecGetNumRhsEvals``
"""""""""""""""""""""""""""""""
:Definition: ``int ARKBandPrecGetNumRhsEvals(void *arkode_mem, long int *nfevalsBP)`` 
...
``ARKLocalFn``) which approximates the right-hand side function
:math:`g(t,y) \approx f_I(t,y)` and which is computed locally, and an
optional function ``cfn`` (of type ``ARKCommFn``) which performs all
...
:ref:`Fcn.ARKodeSetUserData` and that was passed to the user's
...
``ARKLocalFn``
"""""""""""""""""""""""""""""""
:Definition: ``typedef int (*ARKLocalFn)(long int Nlocal, realtype t,
   N_Vector y, N_Vector glocal, void *user_data)`` 
...
:Arguments:  ``Nlocal`` -- the local vector length
   ...
   ``user_data`` -- a pointer to user data, the same as the
   ``user_data`` parameter passed to :ref:`Fcn.ARKodeSetUserData`.

:Return value:  An ``ARKLocalFn`` should return 0 if successful, a
   positive value if a recoverable error occurred (in which case
   ``ARKode`` will attempt to correct), or a negative value if it
   failed unrecoverably (in which case the integration is halted and
   :ref:`Fcn.ARKode` will return ``ARK_LSETUP_FAIL``).
...
``ARKCommFn``
"""""""""""""""""""""""""""""""
:Definition: ``typedef int (*ARKCommFn)(long int Nlocal, realtype t, N_Vector y, void *user_data)`` 
...
:Notes:  The ``cfn`` function is expected to save communicated data in
   ...
   NULL`` can be passed in the call to :ref:`Fcn.ARKBBDPrecInit` (see
   below).
...
8. Initialize the ``ARKBBDPRE`` preconditioner module 
   ...
   ``flag = ARKBBDPrecInit(arkode_mem, local_N, mudq, mldq, mukeep, mlkeep, dqrely, gloc, cfn);``

   to allocate memory and initialize the internal preconditioner
   data. The last two arguments of :ref:`Fcn.ARKBBDPrecInit` are the
   two user-supplied functions of type :ref:`Fcn.ARKLocalFn` and
   :ref:`Fcn.ARKCommFn` described above, respectivelyl. 
...
11. `Get optional outputs`
    ...
    available by way of the two routines described below,
    :ref:`Fcn.ARKBBDPrecGetWorkSpace` and
    :ref:`Fcn.ARKBBDPrecGetNumGfnEvals`. 
...
``ARKBBDPrecInit``
"""""""""""""""""""""""""""""""
:Definition: ``int ARKBBDPrecInit(void *arkode_mem, long int Nlocal,
...
:Arguments: ``arkode_mem`` -- pointer to the ``ARKode`` memory block.
   ...
   ``gloc`` -- the name of the C function (of type :ref:`Fcn.ARKLocalFn`)
   which computes the approximation :math:`g(t,y) \approx f_I(t,y)`.

   ``cfn`` -- the name of the C function (of type ``ARKCommFn``) which
...
:Notes:  If one of the half-bandwidths ``mudq`` or ``mldq`` to be used
   ...
   negative or exceeds the value ``local_N-1``, it is replaced by 0 or
...
linear solver choice, provided there is no change in ``local_N``,
``mukeep``, or ``mlkeep``. After solving one problem, and after
calling :ref:`Fcn.ARKodeReInit` to re-initialize ``ARKode`` for a
subsequent problem, a call to :ref:`Fcn.ARKBBDPrecReInit` can be made
...
inputs, an additional call to :ref:`Fcn.ARKSpgmr`,
:ref:`Fcn.ARKSpbcg`, or :ref:`Fcn.ARKSptfqmr`, and/or one or more of
the corresponding ``ARKSpils*Set*`` functions, must also be made (in
the proper order).



.. _Fcn.ARKBBDPrecReInit:

``ARKBBDPrecReInit``
"""""""""""""""""""""""""""""""
:Definition: ``int ARKBBDPrecReInit(void *arkode_mem, long int mudq, long int mldq, realtype dqrely)`` 
...
.. _Fcn.ARKBBDPrecGetWorkSpace:

``ARKBBDPrecGetWorkSpace``
"""""""""""""""""""""""""""""""
:Definition: ``int ARKBBDPrecGetWorkSpace(void *arkode_mem, long int *lenrwBBDP, long int *leniwBBDP)`` 
...
:Notes:  In terms of ``local_N`` and ``smu = min(local_N-1,
   mukeep+mlkeep)``, the actual size of the real workspace is ``(2
   mlkeep + mukeep + smu + 2)*local_N``  ``realtype`` words, and the
   actual size of the integer workspace is ``local_N`` integer
...
.. _Fcn.ARKBBDPrecGetNumGfnEvals:

``ARKBBDPrecGetNumGfnEvals``
"""""""""""""""""""""""""""""""

:Definition: ``int ARKBBDPrecGetNumGfnEvals(void *arkode_mem,
   long int *ngevalsBBDP)`` 

:Description:  Returns the number of calls made to the user-supplied
   ``gloc`` function (of type :ref:`Fcn.ARKLocalFn`) due to the finite

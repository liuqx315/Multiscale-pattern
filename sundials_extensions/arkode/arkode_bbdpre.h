 ...
 * matrix with banded blocks, for use with ARKSPGMR/ARKSPBCG/ARKSPTFQMR, 
 ...
 *    integrator/ARKSPGMR/ARKSPBCG/ARKSPTFQMR optional outputs.
 * -----------------------------------------------------------------
 ***** UNTOUCHED *****
 * -----------------------------------------------------------------
 */
...
typedef int (*ARKCommFn)(long int Nlocal, realtype t, N_Vector y,
                        void *user_data);
 ...
 * ARKSPBCG/ARKBBDPRE or ARKSPTFQMR/ARKBBDPRE provided there is no change 
 * in Nlocal, mukeep, or mlkeep. After solving one problem, and after 
 * calling ARKodeReInit to re-initialize the integrator for a subsequent 
...
SUNDIALS_EXPORT int ARKBBDPrecReInit(void *arkode_mem, long int mudq, long int mldq,
				    realtype dqrely);
 ...
 * -----------------------------------------------------------------
 * ARKBBDPrecGetWorkSpace returns the BBDPRE real and integer work space
 *                       sizes.
...
SUNDIALS_EXPORT int ARKBBDPrecGetWorkSpace(void *arkode_mem, long int *lenrwLS, long int *leniwLS);
SUNDIALS_EXPORT int ARKBBDPrecGetNumGfnEvals(void *arkode_mem, long int *ngevalsBBDP);

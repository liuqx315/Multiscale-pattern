 ...
 *
 * lmm:   The user of the ARKODE package specifies whether to use the
 *        ARK_ADAMS (Adams-Moulton) or ARK_BDF (Backward Differentiation
 *        Formula) linear multistep method. The BDF method is
 *        recommended for stiff problems, and the ARK_ADAMS method is
 *        recommended for nonstiff problems.
 *
 * iter:  At each internal time step, a nonlinear equation must
 *        be solved. The user can specify either ARK_FUNCTIONAL
 *        iteration, which does not require linear algebra, or a
 *        ARK_NEWTON iteration, which requires the solution of linear
 *        systems. In the ARK_NEWTON case, the user also specifies a
 *        ARKODE linear solver. ARK_NEWTON is recommended in case of
 *        stiff problems.
 *
 ...
 * system y' = f(t,y) must have type ARKRhsFn.
 ...
 * user data, the same as that passed to ARKodeSetUserData.
 * 
...
typedef void (*ARKErrHandlerFn)(int error_code, 
...
SUNDIALS_EXPORT void *ARKodeCreate(int lmm, int iter);
 ...
 *                         | [NULL]
 *                         |
 * ARKodeSetMaxOrd          | maximum lmm order to be used by the
 *                         | solver.
 ...
 *                         | [10]
 *                         |
 * ARKodeSetStabLimDet      | flag to turn on/off stability limit
 *                         | detection (TRUE = on, FALSE = off).
 *                         | When BDF is used and order is 3 or
 *                         | greater, ARKsldet is called to detect
 ...
 * -----------------------------------------------------------------
 *                         |
 * ARKodeSetIterType        | Changes the current nonlinear iteration
 *                         | type.
 *                         | [set by ARKodecreate]
 *                         |
 * -----------------------------------------------------------------
...
SUNDIALS_EXPORT int ARKodeSetMaxOrd(void *arkode_mem, int maxord);
...
SUNDIALS_EXPORT int ARKodeSetStabLimDet(void *arkode_mem, booleantype stldet);
...
SUNDIALS_EXPORT int ARKodeSetIterType(void *arkode_mem, int iter);
 ...
 *  ARK_ILL_INPUT f an argument has an illegal value.
 * -----------------------------------------------------------------
 */

SUNDIALS_EXPORT int ARKodeInit(void *arkode_mem, ARKRhsFn f, realtype t0, N_Vector y0);
 ...
 *       yout=y(*tret).
 *
 * itask is ARK_NORMAL or ARK_ONE_STEP. These two modes are described above.
 *
...
SUNDIALS_EXPORT int ARKode(void *arkode_mem, realtype tout, N_Vector yout,
 ...
 *
 *   ARK_BAD_K:    k is not in the range 0, 1, ..., qu.
 *
 ...
 * ARKodeGetLastOrder returns the order used during the last
 *                   internal step
 * ARKodeGetCurrentOrder returns the order to be used on the next
 *                      internal step
 * ARKodeGetNumStabLimOrderReds returns the number of order
...
SUNDIALS_EXPORT int ARKodeGetLastOrder(void *arkode_mem, int *qlast);
SUNDIALS_EXPORT int ARKodeGetCurrentOrder(void *arkode_mem, int *qcur);
SUNDIALS_EXPORT int ARKodeGetNumStabLimOrderReds(void *arkode_mem, long int *nslred);

 ...
 * -----------------------------------------------------------------
 * This file (companion of fnvector_parallel_ghost.h) contains the 
 * implementation needed for the Fortran initialization of parallel 
 * ghosted vector operations.
 * -----------------------------------------------------------------
 */
...
#include "fnvector_parallel_ghost.h"
		...
		long int *glob_len, int *ier)
{
  ...
  *ier = 0;
  ...
  case FCMIX_CVODE:
    F2C_CVODE_vec = NULL;
    F2C_CVODE_vec = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_len, dim_alen, 
					       dim_off, *F_ordering, *glob_len);
    if (F2C_CVODE_vec == NULL) *ier = -1;
    break;
  case FCMIX_IDA:
    F2C_IDA_vec = NULL;
    F2C_IDA_vec = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_len, dim_alen, 
					       dim_off, *F_ordering, *glob_len);
    if (F2C_IDA_vec == NULL) *ier = -1;
    break;
  case FCMIX_KINSOL:
    F2C_KINSOL_vec = NULL;
    F2C_KINSOL_vec = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_len, dim_alen, 
					       dim_off, *F_ordering, *glob_len);
		  ...
		  long int *glob_lenQ, int *ier)
{
  ...
  *ier = 0;
  ...
  case FCMIX_CVODE:
    F2C_CVODE_vecQ = NULL;
    F2C_CVODE_vecQ = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_lenQ, dim_alenQ, 
						dim_offQ, *F_ordering, *glob_lenQ);
    if (F2C_CVODE_vecQ == NULL) *ier = -1;
    break;
  case FCMIX_IDA:
    F2C_IDA_vecQ = NULL;
    F2C_IDA_vecQ = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_lenQ, dim_alenQ, 
					      dim_offQ, *F_ordering, *glob_lenQ);
		  ...
		  long int *glob_lenB, int *ier)
{
  ...
  *ier = 0;
  ...
  case FCMIX_CVODE:
    F2C_CVODE_vecB = NULL;
    F2C_CVODE_vecB = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_lenB, dim_alenB, 
						dim_offB, *F_ordering, *glob_lenB);
    if (F2C_CVODE_vecB == NULL) *ier = -1;
    break;
  case FCMIX_IDA:
    F2C_IDA_vecB = NULL;
    F2C_IDA_vecB = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_lenB, dim_alenB, 
					      dim_offB, *F_ordering, *glob_lenB);
		   ...
		   long int *glob_lenQB, int *ier)
{
  ...
  *ier = 0;
  ...
  case FCMIX_CVODE:
    ...
    F2C_CVODE_vecQB = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_lenQB, dim_alenQB, 
						 dim_offQB, *F_ordering, *glob_lenQB);
    if (F2C_CVODE_vecQB == NULL) *ier = -1;
    break;
  case FCMIX_IDA:
    F2C_IDA_vecQB = NULL;
    F2C_IDA_vecQB = N_VNewEmpty_Parallel_Ghost(F2C_comm, *dims, dim_lenQB, dim_alenQB, 
					       dim_offQB, *F_ordering, *glob_lenQB);
...
void FNV_INITPG_S(int *code, long int *Ns, int *ier)
{
  *ier = 0;
  ...
  case FCMIX_CVODE:
    F2C_CVODE_vecS = NULL;
    F2C_CVODE_vecS = (N_Vector *) N_VCloneVectorArrayEmpty_Parallel_Ghost(*Ns, F2C_CVODE_vec);
    if (F2C_CVODE_vecS == NULL) *ier = -1;
    break;
  case FCMIX_IDA:
    F2C_IDA_vecS = NULL;
    F2C_IDA_vecS = (N_Vector *) N_VCloneVectorArrayEmpty_Parallel_Ghost(*Ns, F2C_IDA_vec);

...
-----------------------------------------------------------------
 ...
 Test 1:  Tf=1.5, u0=-2, v0=-2.3553013976081, ep=1.0e-3, ep2=1.0
    Here, v quickly increases to around 0.5, then both change 
    very slowly until around t=0.6, when v increases 
    exponentially to a value of near 12, before it suddenly drops
    down to around -1.0. When v drops, u jumps to around 2.0.  
    Both proceed slowly afterwards for the remainder of the time
    interval.

 Test 2:  Tf=12,  u0=2,  v0=0,  ep=ep2=0.2
    Here, both u and v proceed smoothly until around t=5, when v 
    dips to -7.0 and then returns to around 0.5 within an 
    interval of around 1 unit time.  During that change, u drops 
    from around 1.0 to around -2.0.  This process then repeats 
    but in the opposite direction at around t=11.
...
int main() {
  ...
  /* read problem parameter from input file:

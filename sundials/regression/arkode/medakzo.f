...
c     http://www.dm.uniba.it/~testset/src/problems/medakzo.f
c
c     This is revision
c     $Id: medakzo.F,v 1.3 2006/10/06 12:12:27 testset Exp $
c
c
...
c-----------------------------------------------------------------------
     ...
     +                ind)
      ...
      double precision t(0:2)
      logical numjac, nummas

      fullnm = 'Medical Akzo Nobel problem'
      problm = 'medakzo'
      type   = 'ODE'
      neqn   = 400
      ndisc  = 1
...
c-----------------------------------------------------------------------
      ...
      integer N,i,j
      double precision zeta,dzeta,dzeta2,k,c,phi,alpha,beta,dum
...
c-----------------------------------------------------------------------
      ...
      integer N,i,j
      double precision zeta,dzeta,k,c,phi,alpha,dum
      parameter(k=100d0,c=4d0)

      N      = neqn/2
      dzeta  = 1.d0/dble(N)
      dum    = (dzeta-1.d0)*(dzeta-1.d0)/c
      alpha  = 2.d0*(dzeta-1.d0)*dum/c
...
c     we explicitly evolve the advection and reaction terms
      f(1) = alpha*(y(3)-phi)/(2d0*dzeta)-k*y(1)*y(2)
      f(2) = -k*y(1)*y(2)
c
      do 10 j=2,N-1
         i     = 2*j-1
         zeta  = j*dzeta
         dum   = (zeta-1.d0)*(zeta-1.d0)/c
         alpha = 2.d0*(zeta-1.d0)*dum/c
         f(i) = alpha*(y(i+2)-y(i-2))/(2.d0*dzeta)-k*y(i)*y(i+1)
         i     = 2*j
         f(i) = -k*y(i)*y(i-1)
   10 continue
c
...
c-----------------------------------------------------------------------
      ...
      integer N,i,j
      double precision zeta,dzeta,dzeta2,k,c,phi,beta,dum
      parameter(k=100d0,c=4d0)
...
c     we implicitly evolve the diffusion term
      f(1) = (phi-2.d0*y(1)+y(3))*beta/dzeta2
      f(2) = 0.d0
c
      do 10 j=2,N-1
         i     = 2*j-1
         ...
         dum   = (zeta-1.d0)*(zeta-1.d0)/c
         beta  = dum*dum
         f(i)  = (y(i-2)-2.d0*y(i)+y(i+2))*beta/dzeta2
         i     = 2*j
         f(i)  = 0.d0
   10 continue
c
...
c-----------------------------------------------------------------------
      ...
      integer N,i,j
      double precision zeta,dzeta,dzeta2,beta,k,c,dum,bz
      parameter(k=100d0,c=4d0)
c
      do 20 j=1,neqn
         do 10 i=1,5
   ...
   20 continue
      ...
      dfdy(3,1) = -beta*2.d0/dzeta2
      dfdy(1,3) = beta/dzeta2
c
      do 30 j=2,N-1
         i          = 2*j-1
         zeta       = j*dzeta

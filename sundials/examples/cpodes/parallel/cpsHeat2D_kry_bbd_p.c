 ...
 * This version uses the Krylov solver CPSpgmr and BBD
 * preconditioning.
 *
 ...
 * processors.
 *
 * The system is solved with CPODES using the Krylov linear solver
 * CPSPGMR in conjunction with the preconditioner module CPBBDPRE.
...
static void PrintOutput(int id, void *mem, void *P_data, realtype t, N_Vector uu)
{
  ...
  if (id == 0) {
    ...
    ier = CPodeGetLastOrder(mem, &kused);
    check_flag(&ier, "CPodeGetLastOrder", 1, id);
    ier = CPodeGetNumSteps(mem, &nst);
    check_flag(&ier, "CPodeGetNumSteps", 1, id);
    ier = CPodeGetNumNonlinSolvIters(mem, &nni);
    check_flag(&ier, "CPodeGetNumNonlinSolvIters", 1, id);
    ier = CPodeGetNumFctEvals(mem, &nre);
    check_flag(&ier, "CPodeGetNumFctEvals", 1, id);
    ier = CPodeGetLastStep(mem, &hused);
    check_flag(&ier, "CPodeGetLastStep", 1, id);
    ier = CPSpilsGetNumLinIters(mem, &nli);
    check_flag(&ier, "CPSpilsGetNumLinIters", 1, id);
    ier = CPSpilsGetNumFctEvals(mem, &nreLS);
    check_flag(&ier, "CPSpilsGetNumFctEvals", 1, id);
    ier = CPBBDPrecGetNumGfnEvals(P_data, &nge);
    check_flag(&ier, "CPBBDPrecGetNumGfnEvals", 1, id);
    ier = CPSpilsGetNumPrecEvals(mem, &npe);
    check_flag(&ier, "CPSpilsGetPrecEvals", 1, id);
    ier = CPSpilsGetNumPrecSolves(mem, &nps);
    check_flag(&ier, "CPSpilsGetNumPrecSolves", 1, id);
...
static void PrintFinalStats(void *mem)
{
  ...
  CPSpilsGetNumConvFails(mem, &ncfl);

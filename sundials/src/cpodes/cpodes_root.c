...
int CPodeRootInit(void *cpode_mem, int nrtfn, CPRootFn gfun)
{
  CPodeMem cp_mem;
  booleantype allocOK;
  int i;
...
int cpRcheck1(CPodeMem cp_mem)
{
  ...
  tlo = tn;
  ttol = (SUNRabs(tn) + SUNRabs(h))*uround*FUZZ_FACTOR;
  ...
  retval = gfun(tlo, zn[0], zn[1], glo, user_data);
  nge = 1;
  ...
  retval = gfun(tlo, y, zn[1], glo, user_data);
  nge++;
  if (retval != 0) return(CP_RTFUNC_FAIL);
...
int cpRcheck2(CPodeMem cp_mem)
{
  ...
  retval = gfun(tlo, y, yp, glo, user_data);
  nge++;
  if (retval != 0) return(CP_RTFUNC_FAIL);
...
int cpRcheck3(CPodeMem cp_mem)
{
  ...
  /* Set ghi = g(thi) */
  retval = gfun(thi, y, yp, ghi, user_data);
  nge++;
  if (retval != 0) return(CP_RTFUNC_FAIL);
...
static int cpRootfind(CPodeMem cp_mem, realtype ttol)
{
  ...
  loop {
    ...
    retval = gfun(tmid, y, yp, grout, user_data);
    nge++;
    if (retval != 0) return(CP_RTFUNC_FAIL);
